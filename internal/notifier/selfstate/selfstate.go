...
import (
	...
	w "github.com/moira-alert/moira/internal/worker"
)
...
func (selfCheck *SelfCheckWorker) check(nowTS int64, lastMetricReceivedTS, redisLastCheckTS, lastCheckTS, lastRemoteCheckTS, nextSendErrorMessage, metricsCount, checksCount, remoteChecksCount *int64) {
	...
	if *nextSendErrorMessage < nowTS {
		...
		if *lastMetricReceivedTS < nowTS-selfCheck.Config.LastMetricReceivedDelaySeconds && err == nil {
			...
			selfCheck.setNotifierState(moira2.SelfStateERROR)
		}
		...
		if notifierState, _ := selfCheck.DB.GetNotifierState(); notifierState != moira2.SelfStateOK {
...
func appendNotificationEvents(events *[]moira2.NotificationEvent, message string, currentValue int64) {
...
func (selfCheck *SelfCheckWorker) sendErrorMessages(events *[]moira2.NotificationEvent) {
