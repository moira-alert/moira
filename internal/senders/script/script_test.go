...
var (
	testTrigger = moira2.TriggerData{ID: "triggerID"}
	testContact = moira2.ContactData{Type: "contactType", ID: "contactID", Value: "contactValue"}
)
...
var execStringTestCases = []execStringTestCase{
	{
		template: fmt.Sprintf("%s/%s/%s/%s/%s/script.go", testDir, moira2.VariableContactType, moira2.VariableContactID, moira2.VariableTriggerID, moira2.VariableContactValue),
		expected: "/tmp/contactType/contactID/triggerID/contactValue/script.go",
	},
	{
		template: fmt.Sprintf("%s/%s/%s/%s/script.go", testDir, moira2.VariableContactID, moira2.VariableTriggerID, moira2.VariableContactValue),
		expected: "/tmp/contactID/triggerID/contactValue/script.go",
	},
	{
		template: fmt.Sprintf("%s/%s/%s/script.go", testDir, moira2.VariableTriggerID, moira2.VariableContactValue),
		expected: "/tmp/triggerID/contactValue/script.go",
	},
	{
		template: fmt.Sprintf("%s/%s/script.go", testDir, moira2.VariableContactValue),
...
func TestBuildCommandData(t *testing.T) {
	...
	Convey("Test send events", t, func() {
		...
		scriptFile, args, scriptBody, err := sender.buildCommandData([]moira2.NotificationEvent{{Metric: "New metric"}}, moira2.ContactData{ID: "ContactID"}, moira2.TriggerData{ID: "TriggerID"}, true)
