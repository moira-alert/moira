...
func (connector *DbConnector) GetMetricsValues(metrics []string, from int64, until int64) (map[string][]*moira2.MetricValue, error) {
	c := connector.pool.Get()
	defer c.Close()
	...
	res := make(map[string][]*moira2.MetricValue, len(resultByMetrics))
...
func (connector *DbConnector) SaveMetrics(metrics map[string]*moira2.MatchedMetric) error {
	if len(metrics) == 0 {
		return nil
	}
	...
	for _, metric := range metrics {
		...
		for _, pattern := range metric.Patterns {
			...
			event, err := json.Marshal(&moira2.MetricEvent{
...
func (connector *DbConnector) SubscribeMetricEvents(tomb *tomb.Tomb) (<-chan *moira2.MetricEvent, error) {
	metricsChannel := make(chan *moira2.MetricEvent, pubSubWorkerChannelSize)
	...
	go func() {
		for {
			...
			metricEvent := &moira2.MetricEvent{}
