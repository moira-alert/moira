...
func (connector *DbConnector) GetTrigger(triggerID string) (moira2.Trigger, error) {
	c := connector.pool.Get()
	defer c.Close()
	...
	if err != nil {
		return moira2.Trigger{}, fmt.Errorf("failed to EXEC: %s", err.Error())
	}
...
func (connector *DbConnector) GetTriggers(triggerIDs []string) ([]*moira2.Trigger, error) {
	c := connector.pool.Get()
	defer c.Close()
	...
	triggers := make([]*moira2.Trigger, len(triggerIDs))
...
func (connector *DbConnector) SaveTrigger(triggerID string, trigger *moira2.Trigger) error {
	if trigger.IsRemote {
		trigger.Patterns = make([]string, 0)
	}

	var oldTrigger *moira2.Trigger
	...
	if oldTrigger != nil {
		return connector.cleanupPatternsOutOfUse(moira2.GetStringListsDiff(oldTrigger.Patterns, trigger.Patterns))
	}

	return nil
}

func (connector *DbConnector) updateTrigger(triggerID string, newTrigger *moira2.Trigger, oldTrigger *moira2.Trigger) error {
	...
	c.Send("MULTI")
	if oldTrigger != nil {
		for _, pattern := range moira2.GetStringListsDiff(oldTrigger.Patterns, newTrigger.Patterns) {
		...
		for _, tag := range moira2.GetStringListsDiff(oldTrigger.Tags, newTrigger.Tags) {
...
func (connector *DbConnector) removeTrigger(triggerID string, trigger *moira2.Trigger) error {
	c := connector.pool.Get()
	defer c.Close()
...
// If there is no object by current ID, then nil is returned
func (connector *DbConnector) GetTriggerChecks(triggerIDs []string) ([]*moira2.TriggerCheck, error) {
	c := connector.pool.Get()
	defer c.Close()
	...
	triggerChecks := make([]*moira2.TriggerCheck, len(slices))
	for i, slice := range slices {
		...
		triggerChecks[i] = &moira2.TriggerCheck{
			Trigger:    trigger,
			LastCheck:  lastCheck,
...
func (connector *DbConnector) getTriggerWithTags(triggerRaw interface{}, tagsRaw interface{}, triggerID string) (moira2.Trigger, error) {
...
func (connector *DbConnector) triggerHasSubscriptions(trigger *moira2.Trigger) (bool, error) {
	...
	for _, subscription := range subscriptions {
		...
		if subscription.AnyTags || moira2.Subset(subscription.Tags, trigger.Tags) {
			return true, nil
		}
	}
