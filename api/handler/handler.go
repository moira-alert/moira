...
func NewHandler(db moira.Database, log moira.Logger, index moira.Searcher, config *api.Config, metricSourceProvider *metricSource.SourceProvider, webConfig []byte) http.Handler {
	database = db
	searchIndex = index
