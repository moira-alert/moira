...
func (index *Index) SearchTriggers(filterTags []string, searchString string, onlyErrors bool, page int64, size int64, createdBy string) (searchResults []*moira.SearchResult, total int64, err error) {
	if !index.checkIfIndexIsReady() {
		return make([]*moira.SearchResult, 0), 0, fmt.Errorf("index is not ready, please try later")
	}
	return index.triggerIndex.Search(filterTags, searchString, onlyErrors, page, size, createdBy)
}
