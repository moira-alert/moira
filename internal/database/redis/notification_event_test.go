	...
	Convey("Notification events manipulation", t, func() {
		Convey("Test push-get-get count-fetch", func() {
			Convey("Should no events", func() {
				...
				So(err, ShouldBeNil)
				So(actual, ShouldResemble, make([]*moira2.NotificationEvent, 0))
				...
				So(actual1, ShouldResemble, moira2.NotificationEvent{})
			})
			...
			Convey("Should has one events after push", func() {
				...
				actual, err := dataBase.GetNotificationEvents(notificationEvent.TriggerID, 0, 1)
				So(err, ShouldBeNil)
				So(actual, ShouldResemble, []*moira2.NotificationEvent{&notificationEvent})
		...
		Convey("Test push-fetch multiple event by differ triggerIDs", func() {
			Convey("Push events and get it by triggerIDs", func() {
				...
				So(err, ShouldBeNil)
				So(actual, ShouldResemble, []*moira2.NotificationEvent{&notificationEvent1})
				...
				So(err, ShouldBeNil)
				So(actual, ShouldResemble, []*moira2.NotificationEvent{&notificationEvent2})
			...
			Convey("Fetch second then fetch and and check for ErrNil", func() {
				...
				So(err, ShouldBeError)
				So(err, ShouldResemble, database.ErrNil)
				So(actual, ShouldResemble, moira2.NotificationEvent{})
			})
		})
		...
		Convey("Test get by ranges", func() {
			...
			So(err, ShouldBeNil)
			So(actual, ShouldResemble, []*moira2.NotificationEvent{&event})
...
func TestNotificationEventErrorConnection(t *testing.T) {
	...
	var newNotificationEvent = moira2.NotificationEvent{
	...
	Convey("Should throw error when no connection", t, func() {
		...
		So(actual2, ShouldResemble, moira2.NotificationEvent{})
		So(err, ShouldNotBeNil)
	})
}

var notificationEvent = moira2.NotificationEvent{
	Timestamp: time.Now().Unix(),
	State:     moira2.StateNODATA,
	OldState:  moira2.StateNODATA,
	TriggerID: "81588c33-eab3-4ad4-aa03-82a9560adad9",
...
var notificationEvent1 = moira2.NotificationEvent{
	Timestamp: time.Now().Unix(),
...
var notificationEvent2 = moira2.NotificationEvent{
	Timestamp: time.Now().Unix(),
