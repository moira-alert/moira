...
type Storage struct {
	...
	metricsCache    map[string]*moira2.MatchedMetric
	logger          moira2.Logger
}

// NewCacheStorage create new Storage
func NewCacheStorage(logger moira2.Logger, metrics *metrics.FilterMetrics, reader io.Reader) (*Storage, error) {
	storage := &Storage{
		retentionsCache: make(map[string]*retentionCacheItem),
		metricsCache:    make(map[string]*moira2.MatchedMetric),
		metrics:         metrics,
		logger:          logger,
	}
...
func (storage *Storage) EnrichMatchedMetric(batch map[string]*moira2.MatchedMetric, m *moira2.MatchedMetric) {
	m.Retention = storage.getRetention(m)
	m.RetentionTimestamp = moira2.RoundToNearestRetention(m.Timestamp, int64(m.Retention))
...
func (storage *Storage) getRetention(m *moira2.MatchedMetric) int {
