...
func (connector *DbConnector) GetTriggerLastCheck(triggerID string) (moira2.CheckData, error) {
...
func (connector *DbConnector) SetTriggerLastCheck(triggerID string, checkData *moira2.CheckData, isRemote bool) error {
...
func (connector *DbConnector) SetTriggerCheckMaintenance(triggerID string, metrics map[string]int64, triggerMaintenance *int64, userLogin string, timeCallMaintenance int64) error {
	...
	for readingErr != redis.ErrNil {
		var lastCheck = moira2.CheckData{}
		...
		if len(metricsCheck) > 0 {
			for metric, value := range metrics {
				...
				moira2.SetMaintenanceUserAndTime(&data, value, userLogin, timeCallMaintenance)
				metricsCheck[metric] = data
			}
		}
		if triggerMaintenance != nil {
			moira2.SetMaintenanceUserAndTime(&lastCheck, *triggerMaintenance, userLogin, timeCallMaintenance)
		}
...
func (connector *DbConnector) checkDataScoreChanged(triggerID string, checkData *moira2.CheckData) bool {
	c := connector.pool.Get()
	defer c.Close()
