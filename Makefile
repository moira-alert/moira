...
.PHONY: test
test: prepare
	echo 'mode: atomic' > coverage.txt && go list ./... | grep -v "/vendor/" | xargs -n1 -I{} sh -c 'go test -v -bench=. -covermode=atomic -coverprofile=coverage.tmp -parallel=4 {} && tail -n +2 coverage.tmp >> coverage.txt' && rm coverage.tmp
