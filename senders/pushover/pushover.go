...
func (sender *Sender) buildMessage(events moira.NotificationEvents, throttled bool) string {
	...
	for i, event := range events {
		...
		message.WriteString(fmt.Sprintf("%s: %s = %s (%s to %s)", event.FormatTimestamp(sender.location), event.Metric, event.GetMetricsValues(moira.None), event.OldState, event.State))
