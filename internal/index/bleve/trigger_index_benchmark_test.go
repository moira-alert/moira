...
func fillIndexWithTriggers(index *TriggerIndex, triggerChecksToIndex []*moira2.TriggerCheck, batchSize int) {
...
func generateTriggerChecks(number int) []*moira2.TriggerCheck {
	triggersPointers := make([]*moira2.TriggerCheck, number)
	for i := range triggersPointers {
		description := randStringBytes(500)
		triggersPointers[i] = &moira2.TriggerCheck{
...
func chunkTriggerChecks(original []*moira2.TriggerCheck, chunkSize int) (divided [][]*moira2.TriggerCheck) {
	if chunkSize < 1 {
		return
	}
