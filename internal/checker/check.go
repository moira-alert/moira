...
func (triggerChecker *TriggerChecker) checkTrigger() (moira2.CheckData, error) {
...
func newCheckData(lastCheck *moira2.CheckData, checkTimeStamp int64) moira2.CheckData {
	lastMetrics := make(map[string]moira2.MetricState, len(lastCheck.Metrics))
...
func newMetricState(oldMetricState moira2.MetricState, newState moira2.State, newTimestamp int64, newValue *float64) *moira2.MetricState {
...
func (triggerChecker *TriggerChecker) checkTriggerMetrics(triggerMetricsData *metricSource.TriggerMetricsData, checkData moira2.CheckData) (moira2.CheckData, error) {
...
func (triggerChecker *TriggerChecker) checkMetricData(metricData *metricSource.MetricData, triggerMetricsData *metricSource.TriggerMetricsData) (lastState moira2.MetricState, needToDeleteMetric bool, err error) {
...
func (triggerChecker *TriggerChecker) handleCheckResult(checkData moira2.CheckData, checkingError error) (moira2.CheckData, error) {
	if checkingError == nil {
		checkData.State = moira2.StateOK
	...
	case ErrWrongTriggerTargets, ErrTriggerHasSameMetricNames:
		checkData.State = moira2.StateERROR
		checkData.Message = checkingError.Error()
	case remote.ErrRemoteTriggerResponse:
		...
		if timeSinceLastSuccessfulCheck >= triggerChecker.ttl {
			checkData.State = moira2.StateEXCEPTION
...
func (triggerChecker *TriggerChecker) checkForNoData(metricData *metricSource.MetricData, metricLastState moira2.MetricState) (bool, *moira2.MetricState) {
	if triggerChecker.ttl == 0 {
		return false, nil
	}
	...
	if triggerChecker.ttlState == moira2.TTLStateDEL && metricLastState.EventTimestamp != 0 {
		return true, nil
	}
...
func (triggerChecker *TriggerChecker) getMetricStepsStates(triggerMetricsData *metricSource.TriggerMetricsData, metricData *metricSource.MetricData, metricLastState moira2.MetricState) ([]moira2.MetricState, error) {
	...
	metricStates := make([]moira2.MetricState, 0)
...
func (triggerChecker *TriggerChecker) getMetricDataState(triggerMetricsData *metricSource.TriggerMetricsData, metricData *metricSource.MetricData, lastState moira2.MetricState, valueTimestamp, checkPoint int64) (*moira2.MetricState, error) {
...
func getExpressionValues(triggerMetricsData *metricSource.TriggerMetricsData, firstTargetMetricData *metricSource.MetricData, valueTimestamp int64) (*expression.TriggerExpression, bool) {
	...
	if !moira2.IsValidFloat64(firstTargetValue) {
	...
	for targetNumber := 0; targetNumber < len(triggerMetricsData.Additional); targetNumber++ {
		...
		if !moira2.IsValidFloat64(tnValue) {
			return expressionValues, false
		}
