...
func TestMetricsStoring(t *testing.T) {
	...
	Convey("GetPatterns works only if you add new trigger with this pattern", t, func(c C) {
		...
		c.So(err, ShouldBeNil)
		c.So(actualMetric, ShouldHaveLength, 1)
		...
		actualMetric, err = dataBase.GetPatternMetrics(pattern)
		c.So(err, ShouldBeNil)
		c.So(actualMetric, ShouldHaveLength, 2)
		...
		actual, err = dataBase.GetPatterns()
		c.So(err, ShouldBeNil)
		c.So(actual, ShouldResemble, trigger.Patterns)
		...
		Convey("You can remove pattern and metric separately", t, func(c C) {
			err = dataBase.RemovePattern(pattern)
			c.So(err, ShouldBeNil)
		...
		Convey("You can remove remove pattern with metrics in one request", t, func(c C) {
		...
		actualMetric, err = dataBase.GetPatternMetrics(pattern)
		c.So(err, ShouldBeNil)
		c.So(actualMetric, ShouldBeEmpty)
	})

	Convey("Metrics values and retentions manipulation", t, func(c C) {
		...
		c.So(err, ShouldBeNil)
		c.So(actualValues, ShouldResemble, map[string][]*moira.MetricValue{metric1: {&moira.MetricValue{Timestamp: 15, RetentionTimestamp: 10, Value: 1}}})
		...
		actualValues, err = dataBase.GetMetricsValues([]string{metric1}, 10, 99)
		c.So(err, ShouldBeNil)
		c.So(actualValues, ShouldResemble, map[string][]*moira.MetricValue{metric1: {&moira.MetricValue{Timestamp: 15, RetentionTimestamp: 10, Value: 1}, &moira.MetricValue{Timestamp: 22, RetentionTimestamp: 20, Value: 2}}})

		actualValues, err = dataBase.GetMetricsValues([]string{metric1}, 11, 99)
		c.So(err, ShouldBeNil)
		c.So(actualValues, ShouldResemble, map[string][]*moira.MetricValue{metric1: {&moira.MetricValue{Timestamp: 22, RetentionTimestamp: 20, Value: 2}}})
...
func TestRemoveMetricValues(t *testing.T) {
	...
	Convey("Test", t, func(c C) {
		...
		c.So(err, ShouldBeNil) //Save metric with changed retention
		...
		c.So(err, ShouldBeNil)
		c.So(actualValues, ShouldResemble, map[string][]*moira.MetricValue{
			metric1: {
...
func TestMetricSubscription(t *testing.T) {
	...
	metric2 := "my.test.super.metric2"
	pattern := "my.test.*.metric*"
	Convey("Subscription manipulation", t, func(c C) {
		var tomb1 tomb.Tomb
		ch, err := dataBase.SubscribeMetricEvents(&tomb1)
		c.So(err, ShouldBeNil)
		c.So(ch, ShouldNotBeNil)
		...
		tomb1.Go(func() error {
			for {
				...
				if metricEvent.Metric == metric1 {
					Convey("Test", t, func(c C) {
						numberOfChecks++
						c.So(metricEvent, ShouldResemble, &moira.MetricEvent{Pattern: pattern, Metric: metric1})
					})
				}
				if metricEvent.Metric == metric2 {
					Convey("Test", t, func(c C) {
						numberOfChecks++
						c.So(metricEvent, ShouldResemble, &moira.MetricEvent{Pattern: pattern, Metric: metric2})
					})
				}
			}
		})
		...
		c.So(numberOfChecks, ShouldEqual, 3)
	})
}
