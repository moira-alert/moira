...
func TestCheckErrors(t *testing.T) {
	...
	Convey("Fetch error", t, func() {
		...
		dataBase.EXPECT().SetTriggerLastCheck(triggerChecker.triggerID, &lastCheck, triggerChecker.trigger.SourceType).Return(nil)
...
func TestHandleTriggerCheck(t *testing.T) {
	...
	Convey("Handle trigger error remote trigger response", t, func() {
		...
		Convey("and time since last successful check more than ttl", func() {
			...
			expected := moira.CheckData{
				State:                        moira.StateEXCEPTION,
				Message:                      fmt.Sprintf("GraphiteTrigger server unavailable. Trigger is not checked for %d seconds", checkData.Timestamp-checkData.LastSuccessfulCheckTimestamp),
				Timestamp:                    now.Unix(),
