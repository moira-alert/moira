...
func FetchData(database moira2.Database, pattern string, from int64, until int64, allowRealTimeAlerting bool) ([]*types.MetricData, []string, error) {
	...
	if len(metrics) > 0 {
		...
		dataList := map[string][]*moira2.MetricValue{pattern: make([]*moira2.MetricValue, 0)}
...
func unpackMetricsValues(metricsData map[string][]*moira2.MetricValue, retention int64, from int64, until int64, allowRealTimeAlerting bool) map[string][]float64 {
...
func unpackMetricValues(metricData []*moira2.MetricValue, until int64, allowRealTimeAlerting bool, getTimeSlot func(int64) int64) []float64 {
	points := make(map[int64]*moira2.MetricValue, len(metricData))
...
func getMathFloat64(val *moira2.MetricValue, ok bool) float64 {
	if ok {
		return val.Value
	}
	return math.NaN()
