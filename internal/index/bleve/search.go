...
// Relevance (asc)
// Trigger.Name (asc)
func (index *TriggerIndex) Search(filterTags []string, searchString string, onlyErrors bool, page int64, size int64) (searchResults []*moira2.SearchResult, total int64, err error) {
	if size < 0 {
		page = 0
	...
	for _, result := range searchResult.Hits {
		...
		triggerSearchResult := moira2.SearchResult{
			ObjectID:   result.ID,
...
func getHighlights(fragmentsMap search.FieldFragmentMap, triggerFields ...mapping.FieldData) []moira2.SearchHighlight {
	...
	for _, triggerField := range triggerFields {
		...
		if fragments, ok := fragmentsMap[triggerField.GetName()]; ok {
			...
			highlights = append(highlights, moira2.SearchHighlight{
