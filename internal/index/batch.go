...
func (index *Index) writeByBatches(triggerIDs []string, batchSize int) error {
	triggerIDsBatches := moira2.ChunkSlice(triggerIDs, batchSize)
...
func (index *Index) getTriggerChecksBatches(triggerIDsBatches [][]string) (triggerChecksChan chan []*moira2.TriggerCheck, errors chan error) {
	wg := sync.WaitGroup{}
	triggerChecksChan = make(chan []*moira2.TriggerCheck)
...
func (index *Index) getTriggerChecksWithRetries(batch []string) ([]*moira2.TriggerCheck, error) {
	var err error
	triesCount := 3
	for i := 1; i <= triesCount; i++ {
		var newBatch []*moira2.TriggerCheck
...
func (index *Index) handleTriggerBatches(triggerChecksChan chan []*moira2.TriggerCheck, getTriggersErrors chan error, toIndex int) error {
