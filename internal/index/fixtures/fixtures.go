...
type fixtureIndexedField struct {
	content    string
	highlights map[string][]moira2.SearchHighlight
}
...
func (it *fixtureIndexedTrigger) GetHighLights(searchString string) []moira2.SearchHighlight {
	highlights := make([]moira2.SearchHighlight, 0)
...
func (its *fixtureIndexedTriggers) ToTriggerChecks() []*moira2.TriggerCheck {
	triggerChecks := make([]*moira2.TriggerCheck, 0)
...
func (its *fixtureIndexedTriggers) ToSearchResults(searchString string) []*moira2.SearchResult {
	searchResults := make([]*moira2.SearchResult, 0)
	for _, indexedTrigger := range its.list {
		searchResult := moira2.SearchResult{
