...
func (connector *DbConnector) refreshUnusedTriggers(newTriggers, oldTriggers []*moira2.Trigger) error {
	unusedTriggerIDs := make([]string, 0)
	usedTriggerIDs := make([]string, 0)

	triggersNotInNewList := moira2.GetTriggerListsDiff(oldTriggers, newTriggers)
