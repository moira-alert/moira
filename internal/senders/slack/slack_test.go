...
func TestGetStateEmoji(t *testing.T) {
	...
	Convey("Use emoji is false", t, func() {
		So(sender.getStateEmoji(moira2.StateERROR), ShouldResemble, "")
	})

	Convey("Use emoji is true", t, func() {
		sender := Sender{useEmoji: true}
		So(sender.getStateEmoji(moira2.StateOK), ShouldResemble, okEmoji)
		So(sender.getStateEmoji(moira2.StateWARN), ShouldResemble, warnEmoji)
		So(sender.getStateEmoji(moira2.StateERROR), ShouldResemble, errorEmoji)
		So(sender.getStateEmoji(moira2.StateNODATA), ShouldResemble, nodataEmoji)
		So(sender.getStateEmoji(moira2.StateEXCEPTION), ShouldResemble, exceptionEmoji)
		So(sender.getStateEmoji(moira2.StateTEST), ShouldResemble, testEmoji)
	})
}
