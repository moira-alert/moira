...
import (
	...
	mock_notifier "github.com/moira-alert/moira/internal/mock/notifier"
...
func TestDatabaseDisconnected(t *testing.T) {
	...
	Convey("Database disconnected", t, func() {
		Convey("Should notify admin", func() {
			var events []moira2.NotificationEvent
			...
			mock.database.EXPECT().GetNotifierState().Return(moira2.SelfStateERROR, err)
			...
			appendNotificationEvents(&events, notifierStateErrorMessage(moira2.SelfStateERROR), 0)
...
func TestMoiraCacheDoesNotReceivedNewMetrics(t *testing.T) {
	...
	Convey("Should notify admin", t, func() {
		...
		mock.database.EXPECT().SetNotifierState(moira2.SelfStateERROR).Return(nil)
		mock.database.EXPECT().GetNotifierState().Return(moira2.SelfStateERROR, nil)
		mock.notif.EXPECT().Send(&expectedPackage, &sendingWG)
...
func TestMoiraCheckerDoesNotChecksRemoteTriggers(t *testing.T) {
	...
	Convey("Should notify admin", t, func() {
		...
		mock.database.EXPECT().GetNotifierState().Return(moira2.SelfStateOK, nil)
		mock.notif.EXPECT().Send(&expectedPackage, &sendingWG)
		mock.selfCheckWorker.check(callingNow.Unix(), &lastMetricReceivedTS, &redisLastCheckTS, &lastCheckTS, &lastRemoteCheckTS, &nextSendErrorMessage, &metricsCount, &checksCount, &remoteChecksCount)
...
func TestRunGoRoutine(t *testing.T) {
	...
	Convey("Go routine run before first send, should send after 10 seconds next time", t, func() {
		...
		database.EXPECT().GetNotifierState().Return(moira2.SelfStateERROR, err).Times(3)
...
func configureNotificationPackage(adminContact map[string]string, events *[]moira2.NotificationEvent) notifier.NotificationPackage {
