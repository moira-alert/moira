...
func TestSetTriggerMaintenance(t *testing.T) {
	...
	Convey("Success setting metrics maintenance only", t, func() {
		dataBase.EXPECT().SetTriggerCheckMaintenance(triggerID, triggerMaintenance.Metrics, triggerMaintenance.Trigger,nil, nil).Return(nil)
		err := SetTriggerMaintenance(dataBase, triggerID, triggerMaintenance,nil,nil)
		So(err, ShouldBeNil)
	})
	...
	Convey("Error", t, func() {
		...
		dataBase.EXPECT().SetTriggerCheckMaintenance(triggerID, triggerMaintenance.Metrics, triggerMaintenance.Trigger,nil,nil).Return(expected)
		err := SetTriggerMaintenance(dataBase, triggerID, triggerMaintenance,nil,nil)
		So(err, ShouldResemble, api.ErrorInternalServer(expected))
	})
}
