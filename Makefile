...
docker_image:
	for service in "filter" "notifier" "api" "checker" ; do \
		docker build --build-arg MoiraVersion=${VERSION} --build-arg GO_VERSION=${GO_VERSION} --build-arg GIT_COMMIT=${GIT_COMMIT} -f Dockerfile.$$service -t moira/$$service:${VERSION} -t moira/$$service:latest . ; \
	done
