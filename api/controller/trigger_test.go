...
func TestUpdateTrigger(t *testing.T) {
	...
	Convey("Success update", t, func() {
		...
		dataBase.EXPECT().SetTriggerLastCheck(gomock.Any(), gomock.Any(), trigger.TriggerSource == moira.GraphiteRemote).Return(nil)
...
func TestSaveTrigger(t *testing.T) {
	...
	Convey("No timeSeries", t, func() {
		Convey("No last check", func() {
			...
			dataBase.EXPECT().SetTriggerLastCheck(triggerID, gomock.Any(), trigger.TriggerSource == moira.GraphiteRemote).Return(nil)
		...
		Convey("Has last check", func() {
			...
			dataBase.EXPECT().SetTriggerLastCheck(triggerID, &emptyLastCheck, trigger.TriggerSource == moira.GraphiteRemote).Return(nil)
			dataBase.EXPECT().SaveTrigger(triggerID, &trigger).Return(nil)
			resp, err := saveTrigger(dataBase, &trigger, triggerID, make(map[string]bool))
			So(err, ShouldBeNil)
			So(resp, ShouldResemble, &dto.SaveTriggerResponse{ID: triggerID, Message: "trigger updated"})
	...
	Convey("Errors", t, func() {
		...
		Convey("SetTriggerLastCheck error", func() {
			...
			dataBase.EXPECT().DeleteTriggerCheckLock(triggerID)
			dataBase.EXPECT().GetTriggerLastCheck(triggerID).Return(moira.CheckData{}, database.ErrNil)
			dataBase.EXPECT().SetTriggerLastCheck(triggerID, gomock.Any(), trigger.TriggerSource == moira.GraphiteRemote).Return(expected)
			resp, err := saveTrigger(dataBase, &trigger, triggerID, make(map[string]bool))
			So(err, ShouldResemble, api.ErrorInternalServer(expected))
			So(resp, ShouldBeNil)
		})
...
func TestVariousTtlState(t *testing.T) {
	...
	Convey("Various TTLState", t, func() {
		Convey("NODATA TTLState", func() {
			...
			dataBase.EXPECT().DeleteTriggerCheckLock(triggerID)
			dataBase.EXPECT().GetTriggerLastCheck(triggerID).Return(moira.CheckData{}, database.ErrNil)
			dataBase.EXPECT().SetTriggerLastCheck(triggerID, &lastCheck, trigger.TriggerSource == moira.GraphiteRemote).Return(nil)
			dataBase.EXPECT().SaveTrigger(triggerID, &trigger).Return(nil)
			resp, err := saveTrigger(dataBase, &trigger, triggerID, make(map[string]bool))
			So(err, ShouldBeNil)
			So(resp, ShouldResemble, &dto.SaveTriggerResponse{ID: triggerID, Message: "trigger updated"})
