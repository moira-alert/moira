...
func TestCreateEmptyFetchResult(t *testing.T) {
	Convey("Just create fetch empty fetch result", t, func(c C) {
		c.So(*(CreateEmptyFetchResult()), ShouldResemble, FetchResult{
...
func TestFetchResult_GetMetricsData(t *testing.T) {
	Convey("Get empty metric data", t, func(c C) {
		fetchResult := &FetchResult{
			MetricsData: make([]*metricSource.MetricData, 0),
			Patterns:    make([]string, 0),
			Metrics:     make([]string, 0),
		}
		c.So(fetchResult.GetMetricsData(), ShouldBeEmpty)
	})

	Convey("Get not empty metric data", t, func(c C) {
		fetchResult := &FetchResult{
			MetricsData: []*metricSource.MetricData{metricSource.MakeMetricData("123", []float64{1, 2, 3}, 60, 0)},
			Patterns:    make([]string, 0),
			Metrics:     make([]string, 0),
		}
		c.So(fetchResult.GetMetricsData(), ShouldHaveLength, 1)
	})
}

func TestFetchResult_GetPatternMetrics(t *testing.T) {
	Convey("Get empty pattern metrics", t, func(c C) {
		fetchResult := &FetchResult{
			MetricsData: make([]*metricSource.MetricData, 0),
			Patterns:    make([]string, 0),
			Metrics:     make([]string, 0),
