...
var plottingData = moira2.PlottingData{
	Enabled: false,
	Theme:   "dark",
}

func TestThrottling(t *testing.T) {
	var trigger = moira2.TriggerData{
	...
	expected := moira2.ScheduledNotification{
		Event:     event,
		Trigger:   trigger,
		Contact:   contact,
	...
	Convey("Test event state is TEST and no send fails, should return now notification time", t, func() {
		subID := "SubscriptionID-000000000000001"
		testEvent := moira2.NotificationEvent{
			Metric:         "generate.event.1",
	...
	Convey("Test no throttling and no subscription, should return now notification time", t, func() {
		...
		dataBase.EXPECT().GetSubscription(*event.SubscriptionID).Times(1).Return(moira2.SubscriptionData{}, fmt.Errorf("Error while read subscription"))
...
var schedule1 = moira2.ScheduleData{
...
var schedule2 = moira2.ScheduleData{
...
var schedule3 = moira2.ScheduleData{
