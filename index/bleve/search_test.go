...
func TestTriggerIndex_Search(t *testing.T) {
	...
	Convey("Search for triggers with pagination", t, func() {
		...
		Convey("No tags, no searchString, onlyErrors = false, page -> 0, size -> 10", func() {
			searchResults, total, err := newIndex.Search(searchOptions)
		...
		Convey("OnlyErrors = false, one tags, no terms, with createdBy, page -> 0, size 5", func() {
			searchOptions.Page = 0
	...
	Convey("Search for triggers by description", t, func() {
		...
		Convey("OnlyErrors = false, search by description, no tags, with createdBy, 3 results", func() {
			searchOptions.SearchString = "mama"
			searchOptions.Tags = make([]string, 0)
