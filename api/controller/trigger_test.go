...
func TestGetAliveMetrics(t *testing.T) {
	Convey("Test getAliveMetrics", t, func() {
		Convey("Test with nil map of metrics", func() {
			aliveMetrics := getAliveMetrics(nil)
			So(aliveMetrics, ShouldResemble, map[string]moira.MetricState{})
		})

		Convey("Test with empty map of metrics", func() {
			aliveMetrics := getAliveMetrics(map[string]moira.MetricState{})
			So(aliveMetrics, ShouldResemble, map[string]moira.MetricState{})
		})
		...
		Convey("Test with all not alive metrics", func() {
			aliveMetrics := getAliveMetrics(map[string]moira.MetricState{
				"test1": {
					DeletedButKept: true,
				},
		...
		Convey("Test with alive and not alive metrics", func() {
			aliveMetrics := getAliveMetrics(map[string]moira.MetricState{
				"test1": {
					DeletedButKept: true,
				},
