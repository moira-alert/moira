...
type Scheduler interface {
	ScheduleNotification(now time.Time, event moira2.NotificationEvent, trigger moira2.TriggerData,
		contact moira2.ContactData, plotting moira2.PlottingData, throttledOld bool, sendfail int) *moira2.ScheduledNotification
}
...
func NewScheduler(database moira2.Database, logger moira2.Logger, metrics *metrics.NotifierMetrics) *StandardScheduler {
...
func (scheduler *StandardScheduler) ScheduleNotification(now time.Time, event moira2.NotificationEvent, trigger moira2.TriggerData,
	contact moira2.ContactData, plotting moira2.PlottingData, throttledOld bool, sendfail int) *moira2.ScheduledNotification {
	var (
		next      time.Time
		throttled bool
...
func (scheduler *StandardScheduler) calculateNextDelivery(now time.Time, event *moira2.NotificationEvent) (time.Time, bool) {
	...
	subscription, err := scheduler.database.GetSubscription(moira2.UseString(event.SubscriptionID))
	if err != nil {
		scheduler.metrics.SubsMalformed.Mark(1)
		scheduler.logger.Debugf("Failed get subscription by id: %s. %s", moira2.UseString(event.SubscriptionID), err.Error())
	...
	if err != nil {
		scheduler.logger.Errorf("Failed to apply schedule for subscriptionID: %s. %s.", moira2.UseString(event.SubscriptionID), err)
	}
	return next, alarmFatigue
}

func calculateNextDelivery(schedule *moira2.ScheduleData, nextTime time.Time) (time.Time, error) {
