...
func (sender *Sender) buildCreateAlertRequest(events moira2.NotificationEvents, trigger moira2.TriggerData, throttled bool, plot []byte, time int64) api.CreateAlertRequest {
...
func (sender *Sender) getMessageType(events moira2.NotificationEvents) api.MessageType {
	msgType := api.Recovery
	for _, event := range events {
		if event.State == moira2.StateERROR || event.State == moira2.StateEXCEPTION {
			msgType = api.Critical
		}
		if msgType != api.Critical && (event.State == moira2.StateWARN || event.State == moira2.StateNODATA) {
			msgType = api.Warning
		}
	}
	return msgType
