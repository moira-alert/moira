...
func TestTriggerStoring(t *testing.T) {
	...
	Convey("Trigger manipulation", t, func() {
		...
		Convey("Test save-get-remove", func() {
			...
			So(actual, ShouldResemble, moira2.Trigger{})
			...
			So(err, ShouldBeNil)
			So(actualTriggers, ShouldResemble, []*moira2.Trigger{trigger})
			...
			So(err, ShouldBeNil)
			So(actualTriggers, ShouldResemble, []*moira2.Trigger{nil})
		...
		Convey("Save trigger with lastCheck and throttling and GetTriggerChecks", func() {
			trigger := triggers[5]
			triggerCheck := &moira2.TriggerCheck{
				Trigger: trigger,
			}
			...
			So(err, ShouldBeNil)
			So(actualTriggerChecks, ShouldResemble, []*moira2.TriggerCheck{triggerCheck})
			...
			actualTriggerChecks, err = dataBase.GetTriggerChecks([]string{trigger.ID})
			So(err, ShouldBeNil)
			So(actualTriggerChecks, ShouldResemble, []*moira2.TriggerCheck{nil})
		})
		...
		Convey("Save trigger with metrics and get metrics", func() {
			...
			actualTriggers, err := dataBase.GetTriggers(ids)
			So(err, ShouldBeNil)
			So(actualTriggers, ShouldResemble, []*moira2.Trigger{triggerVer1})
			...
			actualTriggers, err = dataBase.GetTriggers(ids)
			So(err, ShouldBeNil)
			So(actualTriggers, ShouldResemble, []*moira2.Trigger{triggerVer2})

			//Save metrics for a new pattern metrics
			err = dataBase.SaveMetrics(map[string]*moira2.MatchedMetric{metric2: val2})
			So(err, ShouldBeNil)

			//And check it
			actualValues, err = dataBase.GetMetricsValues([]string{metric2}, 0, 100)
			So(err, ShouldBeNil)
			So(actualValues, ShouldResemble, map[string][]*moira2.MetricValue{metric2: {
				&moira2.MetricValue{
			...
			actualTriggers, err = dataBase.GetTriggers(ids)
			So(err, ShouldBeNil)
			So(actualTriggers, ShouldResemble, []*moira2.Trigger{})
...
func TestTriggerErrorConnection(t *testing.T) {
	...
	Convey("Should throw error when no connection", t, func() {
		...
		So(err, ShouldNotBeNil)
		So(actual1, ShouldResemble, moira2.Trigger{})
