...
func TestDeleteTag(t *testing.T) {
	...
	Convey("Test no trigger ids and subscriptions by tag", t, func() {
		...
		database.EXPECT().GetTagsSubscriptions([]string{tag}).Return([]*moira2.SubscriptionData{}, nil)
	...
	Convey("check create error if subscription exists", t, func() {
		data := []*moira2.SubscriptionData{{ID: "TestSubscription"}}
		database.EXPECT().GetTagTriggerIDs(tag).Return(nil, nil)
...
func TestGetAllTagsAndSubscriptions(t *testing.T) {
	...
	Convey("Success get tag stats", t, func() {
		...
		database.EXPECT().GetTagsSubscriptions([]string{"tag21"}).Return([]*moira2.SubscriptionData{{Tags: []string{"tag21"}}}, nil)
		database.EXPECT().GetTagTriggerIDs("tag21").Return([]string{"trigger21"}, nil)
		database.EXPECT().GetTagsSubscriptions([]string{"tag22"}).Return(make([]*moira2.SubscriptionData, 0), nil)
		database.EXPECT().GetTagTriggerIDs("tag22").Return([]string{"trigger22"}, nil)
		database.EXPECT().GetTagsSubscriptions([]string{"tag1"}).Return([]*moira2.SubscriptionData{{Tags: []string{"tag1", "tag2"}}}, nil)
		...
		for _, stat := range stat.List {
			if stat.TagName == "tag21" {
				So(stat, ShouldResemble, dto.TagStatistics{TagName: "tag21", Triggers: []string{"trigger21"}, Subscriptions: []moira2.SubscriptionData{{Tags: []string{"tag21"}}}})
			}
			if stat.TagName == "tag22" {
				So(stat, ShouldResemble, dto.TagStatistics{TagName: "tag22", Triggers: []string{"trigger22"}, Subscriptions: make([]moira2.SubscriptionData, 0)})
			}
			if stat.TagName == "tag1" {
				So(stat, ShouldResemble, dto.TagStatistics{TagName: "tag1", Triggers: make([]string, 0), Subscriptions: []moira2.SubscriptionData{{Tags: []string{"tag1", "tag2"}}}})
			}
		}
	})
