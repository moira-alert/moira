...
import (
	"github.com/moira-alert/moira/internal/index/bleve"
...
type TriggerIndex interface {
	Search(filterTags []string, searchString string, onlyErrors bool, page int64, size int64) (searchResults []*moira2.SearchResult, total int64, err error)
	Write(checks []*moira2.TriggerCheck) error
...
func NewSearchIndex(logger moira2.Logger, database moira2.Database, metricsRegistry metrics.Registry) *Index {
	var err error
	newIndex := Index{
		logger:   logger,
		database: database,
