...
func BuildTitle(events moira.NotificationEvents, trigger moira.TriggerData, frontURI string, throttled bool) string {
	title := fmt.Sprintf("*%s*", events.GetCurrentState(throttled))
