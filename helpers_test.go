...
func TestFilter(t *testing.T) {
	Convey("Test Filter function", t, func() {
		...
		Convey("Filter strings with length > 3", func() {
			input := []string{"go", "gopher", "convey", "test"}
			expected := []string{"gopher", "convey", "test"}
			actual := Filter(input, func(s string) bool {
				return len(s) > 3
			})
			So(actual, ShouldResemble, expected)
		})

		Convey("Filter empty slice", func() {
			input := []int{}
			expected := []int{}
			actual := Filter(input, func(n int) bool {
				return n%2 == 0
		...
		Convey("Filter with all elements matching", func() {
			input := []int{2, 4, 6, 8}
		...
		Convey("Filter with no elements matching", func() {
			input := []int{1, 3, 5, 7}
			expected := []int{}
			actual := Filter(input, func(n int) bool {
				return n%2 == 0
