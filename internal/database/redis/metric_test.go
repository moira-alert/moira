...
func TestMetricsStoring(t *testing.T) {
	...
	Convey("Metrics values and retentions manipulation", t, func() {
		...
		err = dataBase.SaveMetrics(map[string]*moira2.MatchedMetric{metric1: val1})
		So(err, ShouldBeNil)
		...
		So(err, ShouldBeNil)
		So(actualValues, ShouldResemble, map[string][]*moira2.MetricValue{metric1: {}})

		actualValues, err = dataBase.GetMetricsValues([]string{metric1}, 0, 10)
		So(err, ShouldBeNil)
		So(actualValues, ShouldResemble, map[string][]*moira2.MetricValue{metric1: {&moira2.MetricValue{Timestamp: 15, RetentionTimestamp: 10, Value: 1}}})
		...
		err = dataBase.SaveMetrics(map[string]*moira2.MatchedMetric{metric1: val2})
		So(err, ShouldBeNil)
		...
		actualValues, err = dataBase.GetMetricsValues([]string{metric1}, 10, 99)
		So(err, ShouldBeNil)
		So(actualValues, ShouldResemble, map[string][]*moira2.MetricValue{metric1: {&moira2.MetricValue{Timestamp: 15, RetentionTimestamp: 10, Value: 1}, &moira2.MetricValue{Timestamp: 22, RetentionTimestamp: 20, Value: 2}}})

		actualValues, err = dataBase.GetMetricsValues([]string{metric1}, 11, 99)
		So(err, ShouldBeNil)
		So(actualValues, ShouldResemble, map[string][]*moira2.MetricValue{metric1: {&moira2.MetricValue{Timestamp: 22, RetentionTimestamp: 20, Value: 2}}})
		...
		err = dataBase.SaveMetrics(map[string]*moira2.MatchedMetric{metric1: val3})
		So(err, ShouldBeNil)
...
func TestRemoveMetricValues(t *testing.T) {
	...
	Convey("Test", t, func() {
		err := dataBase.SaveMetrics(map[string]*moira2.MatchedMetric{metric1: met1})
		So(err, ShouldBeNil) //Save metric with changed retention
		err = dataBase.SaveMetrics(map[string]*moira2.MatchedMetric{metric1: met2})
		So(err, ShouldBeNil) //Save metric with changed retention
		err = dataBase.SaveMetrics(map[string]*moira2.MatchedMetric{metric1: met3})
		So(err, ShouldBeNil) //Save metric with changed retention
		err = dataBase.SaveMetrics(map[string]*moira2.MatchedMetric{metric1: met4})
		So(err, ShouldBeNil)

		actualValues, err := dataBase.GetMetricsValues([]string{metric1}, 1, 99)
		So(err, ShouldBeNil)
		So(actualValues, ShouldResemble, map[string][]*moira2.MetricValue{
			metric1: {
				&moira2.MetricValue{Timestamp: 15, RetentionTimestamp: 10, Value: 1},
				&moira2.MetricValue{Timestamp: 24, RetentionTimestamp: 20, Value: 2},
				&moira2.MetricValue{Timestamp: 34, RetentionTimestamp: 30, Value: 3},
				&moira2.MetricValue{Timestamp: 46, RetentionTimestamp: 40, Value: 4},
			},
		})
		...
		tomb1.Go(func() error {
			for {
				...
				if metricEvent.Metric == metric1 {
					Convey("Test", t, func() {
						numberOfChecks++
						So(metricEvent, ShouldResemble, &moira2.MetricEvent{Pattern: pattern, Metric: metric1})
					})
				}
				if metricEvent.Metric == metric2 {
					Convey("Test", t, func() {
						numberOfChecks++
						So(metricEvent, ShouldResemble, &moira2.MetricEvent{Pattern: pattern, Metric: metric2})
					})
				}
			}
		})

		dataBase.SaveMetrics(map[string]*moira2.MatchedMetric{metric1: met1})
		dataBase.SaveMetrics(map[string]*moira2.MatchedMetric{metric2: met2})
		tomb1.Kill(nil)
		tomb1.Wait()
...
func TestMetricsStoringErrorConnection(t *testing.T) {
	...
	Convey("Should throw error when no connection", t, func() {
		...
		err = dataBase.SaveMetrics(map[string]*moira2.MatchedMetric{"metric1": {Value: 1, RetentionTimestamp: 1, Timestamp: 1, Retention: 60, Patterns: []string{"12"}, Metric: "123"}})
		So(err, ShouldNotBeNil)
