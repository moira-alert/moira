...
**bold text** _italics_` + "\n```code```",
		...
		Convey("Print moira message with one event", func() {
			actual := sender.buildMessage([]moira2.NotificationEvent{event}, false, moira2.TriggerData{})
			expected := "02:40: Metric = 123 (OK to NODATA)\n"
			So(actual, ShouldResemble, expected)
		})

		Convey("Print moira message with one event and desc", func() {
			actual := sender.buildMessage([]moira2.NotificationEvent{event}, false, trigger)
		...
		Convey("Print moira message with one event and throttled", func() {
			actual := sender.buildMessage([]moira2.NotificationEvent{event}, true, moira2.TriggerData{})
			expected := `02:40: Metric = 123 (OK to NODATA)
...
func TestMakeCreateAlertRequest(t *testing.T) {
	...
	Convey("Build CreateAlertRequest", t, func() {
		...
		actual := sender.makeCreateAlertRequest(event, contact, trigger, []byte(`test`), false)
