...
func (index *Index) SearchTriggers(filterTags []string, searchString string, onlyErrors bool, page int64, size int64) (searchResults []*moira2.SearchResult, total int64, err error) {
	if !index.checkIfIndexIsReady() {
		return make([]*moira2.SearchResult, 0), 0, fmt.Errorf("index is not ready, please try later")
	}
