...
func (connector *DbConnector) GetSubscription(id string) (moira2.SubscriptionData, error) {
...
func (connector *DbConnector) GetSubscriptions(subscriptionIDs []string) ([]*moira2.SubscriptionData, error) {
	c := connector.pool.Get()
	defer c.Close()
...
func (connector *DbConnector) SaveSubscription(subscription *moira2.SubscriptionData) error {
	var oldSubscription *moira2.SubscriptionData
...
func (connector *DbConnector) updateSubscription(newSubscription *moira2.SubscriptionData, oldSubscription *moira2.SubscriptionData) error {
	c := connector.pool.Get()
	defer c.Close()
...
func (connector *DbConnector) SaveSubscriptions(newSubscriptions []*moira2.SubscriptionData) error {
...
func (connector *DbConnector) updateSubscriptions(oldSubscriptions []*moira2.SubscriptionData, newSubscriptions []*moira2.SubscriptionData) error {
	c := connector.pool.Get()
	defer c.Close()
...
func (connector *DbConnector) RemoveSubscription(subscriptionID string) error {
	...
	if err := connector.refreshUnusedTriggers([]*moira2.Trigger{}, triggers); err != nil {
		return fmt.Errorf("failed to update triggers by subscription: %s", err.Error())
	}
	return nil
}

func (connector *DbConnector) removeSubscription(subscription *moira2.SubscriptionData) error {
	c := connector.pool.Get()
	defer c.Close()
...
func (connector *DbConnector) GetTagsSubscriptions(tags []string) ([]*moira2.SubscriptionData, error) {
...
func addSendSubscriptionRequest(c redis.Conn, subscription moira2.SubscriptionData, oldSubscription *moira2.SubscriptionData) error {
...
func (connector *DbConnector) getSubscriptionTriggers(subscription *moira2.SubscriptionData) ([]*moira2.Trigger, error) {
	if subscription == nil {
		return make([]*moira2.Trigger, 0), nil
	}
...
func (connector *DbConnector) getSubscriptionsTriggers(subscriptions []*moira2.SubscriptionData) ([]*moira2.Trigger, error) {
	triggersMap := make(map[string]*moira2.Trigger)
	triggers := make([]*moira2.Trigger, 0)
