			...
			case <-checkTicker.C:
				if err := worker.processScheduledNotifications(); err != nil {
					...
					case notifierInBadStateError:
						worker.Logger.Warningf(
							"Stop sending notifications for %v: %s. Fix SelfState errors and turn on notifier in /notifications page",
							sleepAfterNotifierBadState, err.Error(),
						)
