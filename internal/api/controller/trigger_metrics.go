...
func GetTriggerEvaluationResult(dataBase moira2.Database, metricSourceProvider *metricSource.SourceProvider, from, to int64, triggerID string, fetchRealtimeData bool) (*metricSource.TriggerMetricsData, *moira2.Trigger, error) {
...
func DeleteTriggerMetric(dataBase moira2.Database, metricName string, triggerID string) *api.ErrorResponse {
...
func DeleteTriggerNodataMetrics(dataBase moira2.Database, triggerID string) *api.ErrorResponse {
...
func GetTriggerMetrics(dataBase moira2.Database, metricSourceProvider *metricSource.SourceProvider, from, to int64, triggerID string) (*dto.TriggerMetrics, *api.ErrorResponse) {
	...
	triggerMetrics := dto.TriggerMetrics{
		Main:       make(map[string][]*moira2.MetricValue),
		Additional: make(map[string][]*moira2.MetricValue),
	}
	for _, timeSeries := range tts.Main {
		values := make([]*moira2.MetricValue, 0)
		for i := 0; i < len(timeSeries.Values); i++ {
			...
			if moira2.IsValidFloat64(value) {
				values = append(values, &moira2.MetricValue{Value: value, Timestamp: timestamp})
			}
		}
...
func deleteTriggerMetrics(dataBase moira2.Database, metricName string, triggerID string, removeAllNodataMetrics bool) *api.ErrorResponse {
	trigger, err := dataBase.GetTrigger(triggerID)
	if err != nil {
		if err == database.ErrNil {
	...
	if removeAllNodataMetrics {
		for metricName, metricState := range lastCheck.Metrics {
			if metricState.State == moira2.StateNODATA {
