...
func TestSelfCheckWorker_check(t *testing.T) {
	Convey("Check should", t, func() {
		Convey("Send notifications to admins only on database error", func() {
			...
			worker.database.EXPECT().SetNotifierState(gomock.Any(), gomock.Any(), gomock.Any()).Return(redisError)
...
func fillDatabase(database *mock_moira_alert.MockDatabase) {
	...
	database.EXPECT().SetNotifierState(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil)
