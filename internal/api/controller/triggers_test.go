...
func TestCreateTrigger(t *testing.T) {
	...
	Convey("Success with triggerID", t, func() {
		...
		dataBase.EXPECT().GetTrigger(triggerModel.ID).Return(moira2.Trigger{}, database.ErrNil)
		dataBase.EXPECT().AcquireTriggerCheckLock(gomock.Any(), 10)
		dataBase.EXPECT().DeleteTriggerCheckLock(gomock.Any())
		dataBase.EXPECT().GetTriggerLastCheck(gomock.Any()).Return(moira2.CheckData{}, database.ErrNil)
		dataBase.EXPECT().SetTriggerLastCheck(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil)
...
func TestGetAllTriggers(t *testing.T) {
	...
	Convey("Has triggers", t, func() {
		...
		triggers := []*moira2.TriggerCheck{{Trigger: moira2.Trigger{ID: triggerIDs[0]}}, {Trigger: moira2.Trigger{ID: triggerIDs[1]}}}
		triggersList := []moira2.TriggerCheck{{Trigger: moira2.Trigger{ID: triggerIDs[0]}}, {Trigger: moira2.Trigger{ID: triggerIDs[1]}}}
	...
	Convey("No triggers", t, func() {
		...
		mockDatabase.EXPECT().GetTriggerChecks(make([]string, 0)).Return(make([]*moira2.TriggerCheck, 0), nil)
		list, err := GetAllTriggers(mockDatabase)
		So(err, ShouldBeNil)
		So(list, ShouldResemble, &dto.TriggersList{List: make([]moira2.TriggerCheck, 0)})
	})
...
func TestSearchTriggers(t *testing.T) {
	...
	var size int64 = 50
	var exp int64 = 31
	testHighlights := make([]moira2.SearchHighlight, 0)
	for field, value := range testHighlightsMap {
		testHighlights = append(testHighlights, moira2.SearchHighlight{
			Field: field,
			Value: value,
		})
	}
	triggerSearchResults := make([]*moira2.SearchResult, 0)
	for _, triggerCheck := range triggerChecks {
		triggerSearchResults = append(triggerSearchResults, &moira2.SearchResult{
			ObjectID:   triggerCheck.ID,
			Highlights: testHighlights,
		})
	}
	triggerIDs := make([]string, len(triggerChecks))
	triggersPointers := make([]*moira2.TriggerCheck, len(triggerChecks))
	for i, trigger := range triggerChecks {
		newTrigger := new(moira2.TriggerCheck)
		*newTrigger = trigger
	...
	Convey("Complex search query", t, func() {
		...
		Convey("Only errors", func() {
			...
			Convey("Only errors with tags and text terms", func() {
				...
				deadlyTraps := []moira2.TriggerCheck{
					triggerChecks[10],
					triggerChecks[14],
					triggerChecks[18],
					triggerChecks[19],
				...
				deadlyTrapsPointers := []*moira2.TriggerCheck{
					&triggerChecks[10],
					&triggerChecks[14],
					&triggerChecks[18],
					&triggerChecks[19],
				...
				deadlyTrapsSearchResults := make([]*moira2.SearchResult, 0)
				for _, deadlyTrap := range deadlyTraps {
					deadlyTrapsSearchResults = append(deadlyTrapsSearchResults, &moira2.SearchResult{
	...
	Convey("Find triggers errors", t, func() {
		...
		Convey("Error from searcher", func() {
			...
			mockIndex.EXPECT().SearchTriggers(tags, searchString, false, page, size).Return(make([]*moira2.SearchResult, 0), int64(0), searcherError)
			list, err := SearchTriggers(mockDatabase, mockIndex, page, size, false, tags, searchString)
...
var triggerChecks = []moira2.TriggerCheck{
	{
