	...
	Convey("When HTTP Response", t, func() {
		Convey("is 200 and body is '1' there should be no error", func() {
			...
			gock.New("https://outlook.office.com/webhook/foo").
				...
				Reply(200).
				BodyString("1")
			contact := moira2.ContactData{Value: "https://outlook.office.com/webhook/foo"}
			err := sender.SendEvents([]moira2.NotificationEvent{event}, contact, trigger, make([]byte, 0, 1), false)
			So(err, ShouldResemble, nil)
...
some other text _italic text_`,
		...
		Convey("Card uses the correct colour for subject state", func() {
			Convey("State is Red for Error", func() {
				actual := sender.buildMessage([]moira2.NotificationEvent{{
					TriggerID: "TriggerID",
					Value:     &value,
					Timestamp: 150000000,
					Metric:    "Metric",
					...
					Message:   nil,
				}}, moira2.TriggerData{Name: "Name"}, false)
