...
func (sender *Sender) buildMessage(events moira.NotificationEvents, throttled bool) string {
	...
	for i, event := range events {
		...
		message.WriteString(fmt.Sprintf("%s: %s = %s (%s to %s)", event.FormatTimestamp(sender.location, moira.DefaultTimeFormat), event.Metric, event.GetMetricsValues(), event.OldState, event.State))
