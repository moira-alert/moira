...
func NewWorker(name string, logger moira2.Logger, lock moira2.Lock, action Action) *Worker {
...
func (worker *Worker) Run(stop <-chan struct{}) {
	for {
		...
		go func(action Action, logger moira2.Logger, done chan struct{}, stop <-chan struct{}) {
			defer close(done)
