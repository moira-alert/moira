	...
	Convey("Init tests", t, func() {
		...
		Convey("has api_token", func() {
			...
			Convey("use_emoji not set", func() {
				...
				sClient := sender.clients[slackType]
				So(sClient, ShouldNotBeNil)
				So(sClient, ShouldResemble, &slackClient{
					logger: logger,
					client: client,
				})
			})
			...
			Convey("use_emoji set to false", func() {
				...
				opts.SenderSettings = senderSettings
...
func TestGetStateEmoji(t *testing.T) {
	...
	Convey("Use emoji is false", t, func() {
		So(client.getStateEmoji(moira.StateERROR), ShouldResemble, "")
	})
	...
	Convey("Use emoji is true", t, func() {
		...
		So(client.getStateEmoji(moira.StateOK), ShouldResemble, okEmoji)
		So(client.getStateEmoji(moira.StateWARN), ShouldResemble, warnEmoji)
		So(client.getStateEmoji(moira.StateERROR), ShouldResemble, errorEmoji)
		So(client.getStateEmoji(moira.StateNODATA), ShouldResemble, nodataEmoji)
		So(client.getStateEmoji(moira.StateEXCEPTION), ShouldResemble, exceptionEmoji)
		So(client.getStateEmoji(moira.StateTEST), ShouldResemble, testEmoji)
	})
}
