...
func (sender *Sender) makeCreateAlertRequest(events moira2.NotificationEvents, contact moira2.ContactData, trigger moira2.TriggerData, plot []byte, throttled bool) *alert.CreateAlertRequest {
...
func (sender *Sender) buildMessage(events moira2.NotificationEvents, throttled bool, trigger moira2.TriggerData) string {
...
func (sender *Sender) buildEventsString(events moira2.NotificationEvents, charsForEvents int, throttled bool) string {
...
func (sender *Sender) getMessagePriority(events moira2.NotificationEvents) alert.Priority {
	priority := alert.P5
	for _, event := range events {
		if event.State == moira2.StateERROR || event.State == moira2.StateEXCEPTION {
			priority = alert.P1
		}
		if priority != alert.P1 && (event.State == moira2.StateWARN || event.State == moira2.StateNODATA) {
			priority = alert.P3
		}
	}
	return priority
