...
func TestDeleteTriggerNodataMetrics(t *testing.T) {
	...
	lastCheckWithManyStates := moira2.CheckData{
		Metrics: map[string]moira2.MetricState{
			"super.metric1": {State: moira2.StateNODATA},
			"super.metric2": {State: moira2.StateNODATA},
			"super.metric3": {State: moira2.StateNODATA},
			"super.metric4": {State: moira2.StateOK},
			"super.metric5": {State: moira2.StateERROR},
			"super.metric6": {State: moira2.StateNODATA},
		},
		Score: 100,
	}

	lastCheckWithoutNodata := moira2.CheckData{
		Metrics: map[string]moira2.MetricState{
			"super.metric4": {State: moira2.StateOK},
			"super.metric5": {State: moira2.StateERROR},
		},
	...
	lastCheckSingleNodata := moira2.CheckData{
		Metrics: map[string]moira2.MetricState{
			"super.metric1": {State: moira2.StateNODATA},
		},
	}
	...
	lastCheckWithNodataOnly := moira2.CheckData{
		Metrics: map[string]moira2.MetricState{
			"super.metric1": {State: moira2.StateNODATA},
			"super.metric2": {State: moira2.StateNODATA},
			"super.metric3": {State: moira2.StateNODATA},
...
func TestGetTriggerMetrics(t *testing.T) {
	...
	Convey("Trigger is remote but remote is not configured", t, func() {
		dataBase.EXPECT().GetTrigger(triggerID).Return(moira2.Trigger{ID: triggerID, Targets: []string{pattern}, IsRemote: true}, nil)
	...
	Convey("Has metrics", t, func() {
		dataBase.EXPECT().GetTrigger(triggerID).Return(moira2.Trigger{ID: triggerID, Targets: []string{pattern}}, nil)
		...
		triggerMetrics, err := GetTriggerMetrics(dataBase, sourceProvider, from, until, triggerID)
		So(err, ShouldBeNil)
		So(*triggerMetrics, ShouldResemble, dto.TriggerMetrics{Main: map[string][]*moira2.MetricValue{metric: {{Value: 0, Timestamp: 17}, {Value: 1, Timestamp: 27}, {Value: 2, Timestamp: 37}, {Value: 3, Timestamp: 47}, {Value: 4, Timestamp: 57}}}, Additional: make(map[string][]*moira2.MetricValue)})
	})
