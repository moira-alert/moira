...
func UpdateTrigger(dataBase moira2.Database, trigger *dto.TriggerModel, triggerID string, timeSeriesNames map[string]bool) (*dto.SaveTriggerResponse, *api.ErrorResponse) {
...
func saveTrigger(dataBase moira2.Database, trigger *moira2.Trigger, triggerID string, timeSeriesNames map[string]bool) (*dto.SaveTriggerResponse, *api.ErrorResponse) {
	...
	if err != database.ErrNil {
		...
		triggerState := moira2.StateNODATA
...
func GetTrigger(dataBase moira2.Database, triggerID string) (*dto.Trigger, *api.ErrorResponse) {
...
func RemoveTrigger(database moira2.Database, triggerID string) *api.ErrorResponse {
...
func GetTriggerThrottling(database moira2.Database, triggerID string) (*dto.ThrottlingResponse, *api.ErrorResponse) {
	throttling, _ := database.GetTriggerThrottling(triggerID)
	throttlingUnix := throttling.Unix()
	if throttlingUnix < time.Now().Unix() {
		throttlingUnix = 0
...
func GetTriggerLastCheck(dataBase moira2.Database, triggerID string) (*dto.TriggerCheck, *api.ErrorResponse) {
	lastCheck := &moira2.CheckData{}
	var err error
...
func DeleteTriggerThrottling(database moira2.Database, triggerID string) *api.ErrorResponse {
	...
	notificationsForRewrite := make([]*moira2.ScheduledNotification, 0)
...
func SetTriggerMaintenance(database moira2.Database, triggerID string, triggerMaintenance dto.TriggerMaintenance, userLogin string, timeCallMaintenance int64) *api.ErrorResponse {
